...
import (
	...
	"github.com/beabys/wms/auth-service-bff/internal/application/auth/usecase"
...
func (a *App) Setup(cfg *Config) error {
	...
	// 2. Dial gRPC client to auth-service
	grpcTarget := fmt.Sprintf("%s:%d", cfg.AuthService.GRPCHost, cfg.AuthService.GRPCPort)
	authClient, err := grpcdapter.NewAuthClient(grpcTarget)
	if err != nil {
	...
	grpcAdapter := grpcdapter.NewAuthServiceAdapter(authClient)

	// 4. Create use case (business logic, validation, transformation)
	authUseCase := usecase.New(log, grpcAdapter)

	// 5. Create HTTP handlers
	server := httpadapter.NewServer(authUseCase, log)
